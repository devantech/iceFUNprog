...
static void help(const char *progname) {
    ...
    fprintf(stderr, "                         or prefix '0x' to the argument "
                    "for size in hexdecimal\n");
    fprintf(stderr, "  --help\n");
...
int main(int argc, char **argv) {
    ...
    if (d == NULL)
               ...
               NULL) // if we were able to read somehting from the directory
        {
            ...
            if (pF) {
                strcpy(portName,
                       portPath); // found iceFUN board so copy full symlink
    ...
    int opt;
    char *endptr;
    while ((opt = getopt_long(argc, argv, "P:o:vh", long_options, NULL)) !=
           -1) {
        switch (opt) {
        case 'P': /* Serial port */
